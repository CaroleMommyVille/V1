...
use Mommy\SecurityBundle\Form\Trim1Form;

// Quarter1 
use Mommy\ProfilBundle\Entity\Quarter1;
...
class Trim1Controller extends Controller implements ContainerAwareInterface
{
	private $step = 'trim1';
	...
	public function getForm() {
		if (is_null($this->form))
			$this->form = $this->createForm(new Trim1Form($this->getDoctrine()->getManager(), $this->container));
		return $this->form;
	}
...
//        $mem->addServer($this->container->getParameter('cache_server'), $this->container->getParameter('cache_port'));
        ...
        $choices['consult1'] = array('oui' => 'Oui', 'non' => 'Non');

        $choices['status'] = $mem->get('security-form-trim1-status');
        if (!$choices['status']) {
            ...
            $mem->set('security-form-trim1-status', $choices['status'], 0);
        }

        $choices['PregnancySymptoms'] = $mem->get('security-form-trim1-symptoms');
        if (!$choices['PregnancySymptoms']) {
            ...
            $mem->set('security-form-trim1-symptoms', $choices['PregnancySymptoms'], 0);
        }

        $choices['PathologyBaby'] = $mem->get('security-form-trim1-pathobb');
        if (!$choices['PathologyBaby']) {
            ...
            $mem->set('security-form-trim1-pathobb', $choices['PathologyBaby'], 0);
        }

        $choices['PathologyPregnancy'] = $mem->get('security-form-trim1-pathogro');
        if (!$choices['PathologyPregnancy']) {
            ...
            $mem->set('security-form-trim1-pathogro', $choices['PathologyPregnancy'], 0);
        }
    ...
    public function handleForm() {
        ...
        $trim1 = $this->form->getData();
        $trim1->setPregnancy($pid);
        ...
        return array(
            'code' => 'ERR_OK',
            'title' => 'Premier trimestre enregistré',
            'message' => $trim1->getId(),
            'data' => null,
        );
    }
